...
```
cmd/<appname>/main.go              # CLI entry point: assembles deps via wiring.NewCore(),
                                   # builds root command, executes it, classifies errors,
                                   # owns os.Exit()
internal/wiring/                   # Configurator: constructs Factory, wires dependencies.
                                   # No CLI framework imports — assembly only.
...
`cmd/<appname>/main.go` is the only file that calls `os.Exit`. It owns the full CLI lifecycle: obtaining the assembled dependency graph from the configurator (`wiring.NewCore`), building the root command, executing it, and classifying errors into exit codes. A local `run()` function keeps `os.Exit` at the top of the call stack so that every `defer` in the call tree executes before the process terminates.
...
import (
    ...
    "mymodule/internal/cmd/root"
    "mymodule/internal/cmdutil"
    "mymodule/internal/wiring"
)
...
The configurator (`internal/wiring/`) assembles infrastructure only — it has no imports from `urfave/cli` or `internal/cmd/`. This separation allows a second executable (e.g., `cmd/np-tui/main.go`) to call the same `wiring.NewCore()` but wire the Factory into a different driving adapter.
