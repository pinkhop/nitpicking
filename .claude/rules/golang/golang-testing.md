...
- Shared test helpers and fakes live in a dedicated package (e.g., `internal/testutil` or `internal/adapters/driven/storage/memory`), not scattered across test files.
...
- Fakes live in a dedicated package (e.g., `internal/adapters/driven/storage/memory`) and are shared across tests.
...
- Gate with `//go:build boundary` build tag.
- Use `testcontainers-go` for database and broker dependencies.
- One `TestMain` per boundary test package to manage container lifecycle.
- Boundary tests still follow one-scenario-per-test. The external system is real; the test structure is identical to unit tests.

## Parallel Execution

- Call `t.Parallel()` in unit tests by default.
- Do not use `t.Parallel()` in boundary tests unless the test is explicitly designed for concurrent execution against shared infrastructure.
