...
`np close --reason` records the reason as a closing comment and closes the issue in one step. Write the reason for a future reader who has not seen the conversation: what changed, what was verified, and any non-obvious caveat.
...
```bash
$ np issue defer --claim a4dace30e46eb1ec14019c79a59c6b27
Deferred FOO-a3bxr
```
