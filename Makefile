...
ifdef VERSION
LDFLAGS := -ldflags "-X $(MODULE)/internal/app.version=$(VERSION)"
else
LDFLAGS :=
endif
...
##
## The version is injected via -ldflags into internal/app.version. When VERSION
...
## test-integration: Run integration tests (may require external systems).
##
## Integration tests verify adapters and boundaries with real external systems
## such as databases and message brokers. They are guarded by the "integration"
## build tag so they do not run during normal development.
.PHONY: test-integration
test-integration:
	go test -tags=integration -run Integration ./...

## test-e2e: Run end-to-end tests (requires a fully configured environment).
##
## E2E tests exercise the full application stack — HTTP endpoints, CLI
## commands, and all wired dependencies. They are guarded by the "e2e" build
## tag and typically run in CI or a staging environment.
.PHONY: test-e2e
test-e2e:
