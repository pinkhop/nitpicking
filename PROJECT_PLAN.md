...
Define `TaskState` enum (`open`, `claimed`, `closed`, `deferred`) and a transition function that enforces legal transitions per §5.1 and §5.3. `closed` is terminal. Return typed errors (from 1.1) for illegal transitions.
...
Define `EpicState` enum (`active`, `claimed`, `deferred`) and a transition function per §5.2 and §5.3. No `closed` state — completion is derived. Return typed errors (from 1.1) for illegal transitions.
...
Task readiness requires: state `open`; no unresolved `blocked_by` (a `blocked_by` target that has been **closed or deleted** counts as resolved); no ancestor epic in `deferred` .

Epic readiness requires: state `active`; no children (needs decomposition); no unresolved `blocked_by` (same resolution rule — closed or deleted targets are resolved); no ancestor epic in `deferred` .
