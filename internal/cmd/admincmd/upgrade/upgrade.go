...
func writeResult(input RunInput, out upgradeOutput) error {
	...
	default:
		...
		_, err := fmt.Fprintf(input.Out,
			"%s Database migrated to v3 (claimed issues converted: %d, history rows removed: %d, legacy relationships translated: %d, idempotency keys migrated: %d, idempotency keys skipped: %d)\n",
			input.ColorScheme.SuccessIcon(),
