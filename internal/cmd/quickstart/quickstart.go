...
func NewCmd(f *cmdutil.Factory) *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			_, _ = fmt.Fprintln(w, "  at the start of every session, and re-provide it whenever context is compacted")
			_, _ = fmt.Fprintln(w, "  or cleared. The prime output is too large for static instruction files.")
			_, _ = fmt.Fprintln(w, "")
			_, _ = fmt.Fprintln(w, "  Your instruction file should contain a brief reference to np so agents know")
			_, _ = fmt.Fprintln(w, "  the tool exists:")
			_, _ = fmt.Fprintf(w, "\n    • %s   — for Claude Code\n", cs.Cyan("CLAUDE.md"))
