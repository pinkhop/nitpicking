...
func ParseRelArg(s string) (RelArgResult, error) {
	...
	default:
		rt, err := issue.ParseRelationType(s)
		if err != nil {
...
func runAddParent(ctx context.Context, input RunAddInput, childID, parentID issue.ID) error {
	if input.ClaimID == "" {
		return fmt.Errorf("--claim is required for parent_of/child_of relationships")
	}

	err := input.Service.UpdateIssue(ctx, service.UpdateIssueInput{
		IssueID:  childID,
