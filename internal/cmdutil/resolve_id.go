...
// IDResolver resolves ticket ID strings that may be either full IDs
...
// Resolve parses a ticket ID string. If the string is a full ID (contains a
...
func (r *IDResolver) Resolve(ctx context.Context, raw string) (ticket.ID, error) {
	...
	if prefixErr != nil {
		...
		return ticket.ID{}, fmt.Errorf("invalid ticket ID %q: %w", raw, err)
	}

	resolved, resolveErr := ticket.ResolveID(raw, prefix)
	if resolveErr != nil {
		return ticket.ID{}, resolveErr
	}
	return resolved, nil
}
