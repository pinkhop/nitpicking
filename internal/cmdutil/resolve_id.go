...
// NewIDResolver creates an IDResolver backed by the given driving. The prefix
...
func (r *IDResolver) Resolve(ctx context.Context, raw string) (domain.ID, error) {
	...
	if prefixErr != nil {
		...
		return domain.ID{}, fmt.Errorf("invalid issue ID %q: %w", raw, err)
	}

	resolved, resolveErr := domain.ResolveID(raw, prefix)
	if resolveErr != nil {
		return domain.ID{}, resolveErr
	}
	return resolved, nil
}
