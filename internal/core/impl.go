...
// Doctor runs all registered diagnostic checks in cascade order, applies
// cascade-skip semantics, and returns the classified findings. MinSeverity
// filtering from input is left to the CLI renderer; the output always
// contains unfiltered Errors and Warnings.
func (s *serviceImpl) Doctor(ctx context.Context, input driving.DoctorInput) (driving.DoctorOutput, error) {
	return runDoctorChecks(ctx, s, doctorRegistry(), input)
}
