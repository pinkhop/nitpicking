...
// ValidateEpicDepth checks that creating an epic under proposedParent would
// not place it at [MaxDepth]. Epics organize children, but an issue at MaxDepth
// cannot have children — so an epic there is structurally invalid. Tasks are
// permitted at MaxDepth because they are leaf nodes.
//
// This validation is additive to [ValidateDepth]; callers should invoke both.
// ValidateDepth rejects any role at depth > MaxDepth, while ValidateEpicDepth
// further restricts epics to depth < MaxDepth.
func ValidateEpicDepth(role domain.Role, proposedParent domain.ID, lookup AncestorLookup) error {
	if role != domain.RoleEpic {
		return nil
	}

	// Count the depth of the proposed parent.
	depth := 1 // proposedParent itself
	current := proposedParent
	...
	// The epic would be at depth+1. It needs room for at least one child,
	// so its depth must be strictly less than MaxDepth.
	childDepth := depth + 1
	if childDepth >= MaxDepth {
		return fmt.Errorf(
			"cannot create epic at level %d: epics organize children, but level %d issues cannot have children (maximum depth is %d): %w",
			childDepth, childDepth, MaxDepth, domain.ErrDepthExceeded,
		)
	}
	return nil
}
