...
func NewComment(p NewCommentParams) (Comment, error) {
	if p.IssueID.IsZero() {
		return Comment{}, NewValidationError("issue_id", "must not be empty")
	}
	if p.Author.IsZero() {
		return Comment{}, NewValidationError("author", "must not be empty")
	}
	if p.Body == "" {
		return Comment{}, NewValidationError("body", "must not be empty")
	}
...
func (c Comment) IssueID() ID { return c.issueID }

// Author returns the comment's author.
func (c Comment) Author() Author { return c.author }
