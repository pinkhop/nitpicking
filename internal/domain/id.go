...
func ParseID(s string) (ID, error) {
	...
	prefix := s[:idx]
	random := NormalizeCrockford(s[idx+1:])
...
func ResolveID(raw, prefix string) (ID, error) {
	...
	if err := validateRandom(normalized); err != nil {
		return ID{}, fmt.Errorf("invalid issue ID %q: %w", raw, err)
	}

	return ParseID(prefix + "-" + normalized)
}
...
// NormalizeCrockford applies Crockford Base32 decoding normalization to s.
// Per the spec (https://www.crockford.com/base32.html), decoding is
// case-insensitive and maps commonly confused characters to their intended
// values:
//   - I/i → 1
//   - L/l → 1
//   - O/o → 0
//
// All other alphabetic characters are lowercased. Digits pass through
// unchanged. The result is suitable for validation against the canonical
// Crockford alphabet.
