...
func Transition(current, next State) error {
	if current.IsTerminal() {
		return fmt.Errorf("cannot transition from %s: %w", current, ErrTerminalState)
	}

	allowed, ok := transitions[current]
	if !ok {
		return fmt.Errorf("unknown state %s: %w", current, ErrIllegalTransition)
	}

	if !allowed[next] {
		return fmt.Errorf("cannot transition from %s to %s: %w", current, next, ErrIllegalTransition)
	}
