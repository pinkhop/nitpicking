...
type IOStreams struct {
	...
	// terminalWidth holds the detected or overridden terminal width.
	// Zero means no width constraint (non-TTY or not yet detected).
	// A positive value from SetTerminalWidth takes precedence over
	// runtime detection.
	terminalWidth    int
	terminalWidthSet bool
}
...
// Returns 0 when stdout is not a TTY (piped output), signaling that
// callers should not apply width-dependent formatting (wrapping,
// truncation). If SetTerminalWidth has been called, the override
// value is returned instead of querying the terminal.
func (s *IOStreams) TerminalWidth() int {
	if s.terminalWidthSet {
		return s.terminalWidth
	}
	if !s.stdoutIsTTY {
		return 0
	}
	width, _, err := term.GetSize(int(os.Stdout.Fd())) // #nosec G115 -- file descriptors are small non-negative integers
	if err != nil {
		return 0
	}
	return width
...
// SetTerminalWidth overrides the detected terminal width. Used in tests
// to simulate a terminal of a specific width without requiring a real TTY.
func (s *IOStreams) SetTerminalWidth(width int) {
	s.terminalWidth = width
	s.terminalWidthSet = true
}
